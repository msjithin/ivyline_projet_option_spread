...
class OptionStrat:
    def __init__(self, name, S0, params=None, start=0, stop=100):
        self.name = name
        self.S0 = S0
        if params:
            ...
            self.STs = np.arange(start, stop, 0.001)
    ...
    def plot(self, **params):
        ...
        ax.plot(self.STs, self.payoffs,**params)
    ...
    def describe(self):
        ...
        out_string = []
        out_string.append( f"Max Profit: ${round(max_profit,3)}" )
        out_string.append(f"Max loss: ${round(max_loss,3)}" )
        c = 0
        for o in self.instruments:
            out_string.append(str(o))
            if o.type == 'call' and o.side==1:
                ...
                c -+ o.price
        c = round(c,2)
        out_string.append(f"Cost of entering position ${ c }")
        return out_string